...
A functions exclusive time is the sum of execution times for all function calls in the program. 
...
public:
    ...
    vector<int> exclusiveTime(int n, vector<string>& logs) {
        ...
        for(string log: logs){
           ...
           struct log item={stoi(tmp1),tmp2,stoi(tmp3)};
            if(item.status=="start"){
                st.push(item);
            }
            else{
                int req_time=item.time-st.top().time+1;
                ans[item.id]+=req_time;
                st.pop();
                if(!st.empty()){
                    ans[st.top().id]-=req_time;
                }
            }
        }
        return ans;
